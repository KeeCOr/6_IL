...
### UI/HUD 목표 안내
- 좌측 하단 웨이브/스탠스 패널은 `OBJECTIVE → RISK → ACTION` 3개 슬롯을 고정 우선순위로 표시한다.
- 밤, 눈보라, 식량 부족처럼 판단이 필요한 상황은 RISK 슬롯을 위험/주의 톤으로 강조하고, ACTION 슬롯은 바로 수행 가능한 행동 문장으로 제한한다.
...
| 버전 | 날짜 | 내용 |
|-----|------|------|
| v0.3.0 | 2026-07-01 | Unity HUD 안내를 OBJECTIVE/RISK/ACTION 슬롯 모델로 분리하고 위험 톤과 고정 우선순위 테스트 추가 |
...
---
## 포트폴리오 리뷰 결정 (2026-07-01)
**결정**: 🔵 별도 개발 진행

### 현황
기획서 v0.3.0 존재. Unity 2D 기반, ECS 프로토타입 진행 중.

### 방침
포트폴리오 리뷰 대상 외 독립 개발 진행. planning-quality-check 6가지 기준으로 기획서 보강 권장.


## 2026-09-07 · 자원 HUD 아이콘화 (v0.4.0)
